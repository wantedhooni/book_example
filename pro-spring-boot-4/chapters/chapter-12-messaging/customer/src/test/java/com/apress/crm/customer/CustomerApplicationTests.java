...
class CustomerApplicationTests extends BaseTest {
	...
	void shouldRetrieveAndCreateCustomers() {
		Customer customer = new Customer("New User", "new@example.com", "555-0199", "password");
	...
	void shouldGetAllCustomers() {
		// Create test data
		customerService.save(new Customer("Alice", "alice@example.com", "555-0001", "password"));
		customerService.save(new Customer("Bob", "bob@example.com", "555-0002", "password"));
		customerService.save(new Customer("Charlie", "charlie@example.com", "555-0003", "password"));
	...
	void shouldGetCustomerById() {
		Customer saved = customerService.save(new Customer("David", "david@example.com", "555-0004", "password"));
	...
	void shouldDeleteCustomer() {
		Customer saved = customerService.save(new Customer("To Delete", "delete@example.com", "555-9999", "password"));
		UUID customerId = saved.getId();
	...
	void shouldRejectInvalidCustomer_MissingName() {
		Customer invalidCustomer = new Customer("", "valid@example.com", "555-0000", "password");
	...
	void shouldRejectInvalidCustomer_InvalidEmail() {
		Customer invalidCustomer = new Customer("Invalid Email User", "not-an-email", "555-0000", "password");
	...
	void shouldVerifyCaching() {
		Customer customer = new Customer("Cached User", "cached@example.com", "555-0200", "password");
	...
	void shouldHandleMultipleSimultaneousRequests() throws InterruptedException {
		...
		Customer customer = new Customer("Concurrent REST User", "concurrent-rest@example.com", "555-9000", "password");
	...
	void shouldVerifyEndToEndWorkflowWithCockroachDB() {
		...
		Customer customer1 = restTestClient.post().uri("/api/v1/customers")
				.body(new Customer("Workflow User 1", "workflow1@example.com", "555-1001", "password"))
				.exchange()
				.expectStatus().isCreated()
				.expectBody(Customer.class)
				.returnResult().getResponseBody();
		...
		Customer customer2 = restTestClient.post().uri("/api/v1/customers")
				.body(new Customer("Workflow User 2", "workflow2@example.com", "555-1002", "password"))
				.exchange()
				.expectStatus().isCreated()
				.expectBody(Customer.class)
				.returnResult().getResponseBody();
	...
	void shouldPersistDataInCockroachDB() {
		...
		// Create customer
		Customer customer = new Customer("Persistence Test", "persist@example.com", "555-7777", "password");
