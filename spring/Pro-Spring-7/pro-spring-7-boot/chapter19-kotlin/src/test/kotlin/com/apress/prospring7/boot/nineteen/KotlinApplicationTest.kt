...
class KotlinApplicationTest(@Autowired val restTemplate: TestRestTemplate, @Value(value = "\${local.server.port}") val port: Int) {
    ...
    fun `find all singers`() {
        ...
        singers?.size?.let { Assertions.assertTrue(it >= 15) }
        if (singers != null) {
    ...
    fun `find Singer by id`() {
        ...
        Assertions.assertAll("testPositiveFindById",
            ...
            { Assertions.assertTrue(Objects.requireNonNull(response.headers[HttpHeaders.CONTENT_TYPE]).contains(MediaType.APPLICATION_JSON_VALUE)) },
            { Assertions.assertNotNull(response.body) },
            { Assertions.assertEquals(Singer::class.java, response.body?.javaClass) }
        )
    }
    ...
    fun `can't find Singer by id`() {
        ...
        Assertions.assertAll("testNegativeFindById",
            ...
            { Assertions.assertNull(response.body?.firstName) },
            { Assertions.assertNull(response.body?.lastName) }
        )
    }
