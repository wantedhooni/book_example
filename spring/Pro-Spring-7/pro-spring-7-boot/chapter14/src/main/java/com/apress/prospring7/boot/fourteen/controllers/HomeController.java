...
SOFTWARE.
*/
package com.apress.prospring7.boot.fourteen.controllers;
...
public class HomeController implements ApplicationContextAware {
    ...
    public String home() {
        return "{\"message\", \"Spring Boot MVC REST!!\"}";
    }

    @GetMapping(path = { "beans"}, produces = MediaType.APPLICATION_JSON_VALUE)
    public List<Map<String,String>> beans() {
        return Arrays.stream(ctx.getBeanDefinitionNames()).sorted()
                . map(name -> Map.of("name", name, "class", ctx.getBean(name).getClass().toGenericString()))
                .toList();
    }
}
